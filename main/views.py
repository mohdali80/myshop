...
from .models import Brand, Category, Product, ProductAttribute, Banner,CartOrder, CartOrderItems, ProductReview, Wishlist,UserAddressBook
...
def product_list(request):
    ...
    #--- We Move cats, brands, colors, and sizes to (template_context.py), this will make us use these for all the website
    #  as a context processor, instead to define these in Product_list, category_product_list and brand_product_list, we
    # define it for one time in the context processor (template_context.py) because it all similar -----> IMPORTANT <-------------------
...
def product_detail(request,slug,id):
    ...
    sizes=ProductAttribute.objects.filter(product=product).values('size__id','size__title','color__id','price').distinct()
...
def filter_data(request):
    ...
    if len(categories)>0:
        allProducts=allProducts.filter(category__title__in=categories).distinct()
    if len(sizes)>0:
...
def checkout(request):
    ...
    if 'cartdata' in request.session:
        ...
        return render(request, 'checkout.html',{'cart_data':request.session['cartdata'],'totalitems':len(request.session['cartdata']),
...
def my_dashboard(request):
    orders=CartOrder.objects.annotate(month=ExtractMonth('order_dt')).values('month').annotate(count
        =Count('id')).values('month','count')
    monthNumber=[]
    totalOrders=[]
    for d in orders:
